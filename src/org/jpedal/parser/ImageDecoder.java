...
public class ImageDecoder extends BaseDecoder{
    ...
    public BufferedImage processImageXObject(PdfObject XObject, String image_name, byte[] objectData, boolean saveRawData, String details) throws PdfException {
        ...
        if(newSMask!=null && index!=null && index.length==3 && decodeColorData.getID()!=ColorSpaces.ICC){ //swap out the image with inverted SMask if empty
    ...
    private void processXImage(String name, String details, String key, PdfObject XObject) throws PdfException {
        ...
        boolean isForHTML=((current.getType()== OutputDisplay.CREATE_HTML || current.getType()== OutputDisplay.CREATE_SVG || current.getType()== OutputDisplay.CREATE_JAVAFX));
        ...
        if (renderImages || finalImagesExtracted || clippedImagesExtracted || rawImagesExtracted) {
            ...
            if (objectData != null || previousUse > 0) {
                ...
                if (image != null || alreadyCached || previousUse > 0) {
                    ...
                    if (!isForHTML &&  (renderDirectly || useHiResImageForDisplay || previousUse > 0)) {
...
//            System.out.println("<<"+i_data.length+" end="+endPtr);
            ...
            if(i_data!=null){
                ...
                if (image != null){
                    ...
                    if((current.getType()== OutputDisplay.CREATE_HTML || current.getType()== OutputDisplay.CREATE_SVG || current.getType()== OutputDisplay.CREATE_JAVAFX)){
                        generateTransformedImage(image, image_name);
                    }else if(renderDirectly || this.useHiResImageForDisplay){// || current.getType()== org.jpedal.render.output.html.HTMLDisplay.CREATE_HTML){
    ...
    public void generateTransformedImage(BufferedImage image,String image_name) {
        ...
        if (image != null) {
            ...
            ImageTransformerDouble image_transformation =new ImageTransformerDouble(gs,image,createScaledVersion);

            //extract images either scaled/clipped or scaled then clipped

            image_transformation.doubleScaleTransformShear(current);
    ...
    BufferedImage processImage(GenericColorSpace decodeColorData,
        ...
        if(current.avoidDownSamplingImage() ||
                ...
                (current.getType()== OutputDisplay.CREATE_HTML || current.getType()== OutputDisplay.CREATE_SVG || current.getType()== OutputDisplay.CREATE_JAVAFX))){
                pX = -1;
                pY=-1;
            }

        /**
    }else if((current.getType()== OutputDisplay.CREATE_HTML || current.getType()== OutputDisplay.CREATE_SVG || current.getType()== OutputDisplay.CREATE_JAVAFX)){

        //float wScale=Math.abs(gs.CTM[0][0]/w);
        //float hScale=Math.abs(gs.CTM[1][1]/h);

        //ignore silly figures
        if(h>4000 && w>4000){// ||wScale<0.1f || hScale<0.1f){
            pX = -1;
            pY=-1;
        ...
        if (image != null) {
            ...
            if((current.getType()== OutputDisplay.CREATE_HTML || current.getType()== OutputDisplay.CREATE_SVG || current.getType()== OutputDisplay.CREATE_JAVAFX)){
            }else if(!renderDirectly && (finalImagesExtracted || rawImagesExtracted) && !cache.testIfImageAlreadySaved(XObject))
