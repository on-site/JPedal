...
public class OutputShape
{
    ...
    public OutputShape(int cmd, float scaling, Shape currentShape, GraphicsState gs, AffineTransform scalingTransform,
	...
	protected void generateShapeFromG2Data(GraphicsState gs, AffineTransform scalingTransform, Rectangle cropBox) {
		...
		int strokeWidth = (int)((gs.getCTMAdjustedLineWidth() * (scaling))+0.99); // It is possible it may be worth dividing the scaling by 1.33f (To get same thickness as PDF @ 100%)
        ...
        switch(pageRotation){
            case 90:
                ...
                for(int i = 0; i<count; i=i+2) {
                    ...
                    if (ctm[0][0] == 0 && ctm[1][1] == 0 && ctm[0][1]>0 &&  (ctm[1][0] < 0||Math.abs(ctm[1][0]) < 0.5)) {//adjust to take account negative shape offset
                        result += setPrecision(((cropBoxH-coords[i+1])*scaling),false);
                        result += ",";
            ...
            case 180:
            {
                ...
                for(int i = 0; i<count; i=i+2) {
                    ...
                    result += setPrecision(((cropBoxH-coords[i+1])*scaling),false);//- (i%2 == 1 ? cropBox.x : cropBox.y) );
                }
            }
            break;
            ...
            case 270:
            {
                ...
                for(int i = 0; i<count; i=i+2) {
                    ...
                    if (ctm[0][0] == 0 && ctm[1][1] == 0 && ctm[0][1]>0 &&  (ctm[1][0] < 0||Math.abs(ctm[1][0]) < 0.5)) {//adjust to take account negative shape offset
                        ...
                        result += setPrecision(((coords[i+1])*scaling),false);
            ...
            default:
            {
                ...
                for(int i = 0; i<count; i=i+2) {
                    ...
                    result += setPrecision((coords[i+1]*scaling),false);//- (i%2 == 1 ? cropBox.x : cropBox.y) );
                }
            }
        }
        return result;
