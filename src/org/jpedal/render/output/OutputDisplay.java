    ...
    private void setParameters(Map params) {
        ...
        if(params==null){
            ...
            if(System.getProperty("org.jpedal.pdf2html.keepOriginalImage")!=null &&
                    System.getProperty("org.jpedal.pdf2html.keepOriginalImage").toLowerCase().equals("true"))
                keepOriginalImage=true;

            if(System.getProperty("org.jpedal.pdf2html.convertOTFFonts")!=null &&
                    System.getProperty("org.jpedal.pdf2html.convertOTFFonts").toLowerCase().equals("true"))
                enableOTFConversion = true;


            isOnlineConverter = System.getProperty("IsOnlineConverter");

            firstPageName = System.getProperty("org.jpedal.pdf2html.firstPageName");

            if(System.getProperty("org.jpedal.pdf2html.stageColor") != null) {
                stageColor = Color.decode(System.getProperty("org.jpedal.pdf2html.stageColor"));
            }
        }else{
            ...
            if((String) params.get("org.jpedal.pdf2html.keepOriginalImage")!=null &&
                    ((String) params.get("org.jpedal.pdf2html.keepOriginalImage")).equals("true"))
                keepOriginalImage=true;

            if((String) params.get("org.jpedal.pdf2html.convertOTFFonts")!=null &&
                    ((String) params.get("org.jpedal.pdf2html.convertOTFFonts")).equals("true"))
                enableOTFConversion = true;

            isOnlineConverter =(String) params.get("IsOnlineConverter");

            firstPageName = (String) params.get("org.jpedal.pdf2html.firstPageName");

            if((String) params.get("org.jpedal.pdf2html.stageColor") != null) {
                stageColor = Color.decode((String) params.get("org.jpedal.pdf2html.stageColor"));
            }
        }
    }
            ...
            case DisplayMode:
                switch(value) {
                    ...
                    case SVG_VIEW_MULTIFILE:
                        ...
                        enableMagazineSplitSpreads = false;
                        enableSinglePageHorizontal = false;
                        break;
                    case HTML_VIEW_MULTIFILE_SPLITSPREADS:
                    case SVG_VIEW_MULTIFILE_SPLITSPREADS:
                        isSingleFileOutput = false;
                        enableMagazineSplitSpreads = true;
                        enableSinglePageHorizontal = false;
                        break;
                    ...
                    case SVG_VIEW_SINGLEFILE_HORIZONTAL:
                        isSingleFileOutput = true;
                        enableMagazineSplitSpreads = false;
                        enableSinglePageHorizontal = true;
                        break;
                }
                break;
...
//                	newTotalHeight = (int)((cropBox.getBounds2D().getHeight()*scaling) + pageGap) * (pageNumber-1) ; //How far down the page the content of next page starts in single file output
    ...
    public void setBooleanValue(int key,boolean value){
        ...
        switch(key){
            ...
            case UseImagesOnNavBar:
                useImagesOnNavBar=value;
                break;

            case UseThumbnailNavbar:
            	useThumbnailNavbar=value;
            	break;
...
//		}
            ...
            case SAVE_EMBEDDED_FONT:
                ...
                if(rawFontData!=null){
                    ...
                    if(type==CREATE_JAVAFX){
                        ...
                        fontTag.append(javaFxFileName + ".class.getResource(\"").append(fileName).append("/fonts/").append(replacedFontName).append('.').append(fileType).append("\").toExternalForm(),10);\n");

                    }else{
                        fontTag.append("@font-face {\n"); //indent
                        fontTag.append("\tfont-family: ").append(replacedFontName).append(";\n");
                        fontTag.append("\tsrc: url(\"").append(fileName).append("/fonts/").append(replacedFontName).append('.').append(fileType).append("\");\n");
                        fontTag.append("}\n");
                    }
    ...
    public int drawImage(int pageNumber, BufferedImage image, GraphicsState gs, boolean alreadyCached, String name, int optionsApplied, int previousUse) {
        ...
        usingCachedImage = false;//reset
        boolean imagesWritten=gs.getClippingShape()==null && imagesAlreadyWritten.get(cacheKey)!=null;
        if(!imagesWritten){
            imagesAlreadyWritten.put(cacheKey,imageId+1);//@Leon TODO - Possible bug here, what if the first image doesn't get output because the cropboxes don't intersect?
        }else if(!embedImageAsBase64){ //except in this case we can ignore image so set to null and just calc coords //@Leon TODO - With my new JS code, we could do this for base64 images.
            usingCachedImage = true;
            cachedImageId = (Integer)imagesAlreadyWritten.get(cacheKey);
            image=null;
        }
        ...
        if(gs.getClippingShape()!=null){
            ...
            float clipW= (float) (clipBounds.getWidth()*scaling);
            float clipH= (float) (clipBounds.getHeight()*scaling);
    	...
    	if(cropBoxScaled.intersects(rect)) {
            ...
            //if(!MARKS_NEW_IMAGE_CODE){  //not sure on whther to use - keeps full hires image
                if(!userControlledImageScaling && !htmlImageMode && image!=null){ //default values

                    if(pageRotation!=0){
                        image = rotateImage(image,pageRotation);
                    }

                    //1 pixel high images also do not scale due to Java bug (see /PDFdata/sample_pdfs_html/general/20120227.pdf)
                    if(image.getHeight()==1 || (!needsflipping && iw==image.getWidth() && ih==image.getHeight())){ //if size correct do not scale
                        savedImage=image;

                    }else{
                        if(g2savedImage==null){
                            savedImage = new BufferedImage((int)iw == 0 ? 1 : (int) iw, (int)ih == 0 ? 1 : (int) ih, image.getType());
...
//                            }
    		...
    		if(embedImageAsBase64){
                ...
                if(!imagesWritten){  //make sure image Dir exists and write out as first time
                    ...
                    customIO.writeImage(rootDir, imageDir+imageId,savedImage);
                    imageName=imageDir+imageId+customIO.getImageTypeUsed();

                } else {
                    imageName=imageDir+cachedImageId+customIO.getImageTypeUsed();
                }
            }

            if (htmlImageMode) {
                // These final values are used by all HTML, SVG, JavaFX, FXML, etc
                currentImage = new int[]{0,0,(int)iw,(int)ih};
            } else {
                // From the PDF spec - If an Image overlaps a pixel by more than 50%, the pixel gets filled, otherwise not.
                int finalX, finalY, finalWidth, finalHeight;
                switch(pageRotation){
                    case 90:
                        double tmpX = pageData.getCropBoxHeight2D(pageNumber)*scaling - coords[1] - iw;
                        double tmpY = coords[0];
...
//        }
        ...
        if(textMode == TEXT_AS_SHAPE ||
                textMode == TEXT_INVISIBLE_ON_IMAGE && getType()== DynamicVectorRenderer.CREATE_SVG ){ //option to convert to shapes
...
//            System.out.println(currentFontData.getBaseFontName() + " " + JSRoutineName + "  fontID= " + safeFontID);
    ...
    public void drawShape(Shape currentShape, GraphicsState gs, int cmd) {
        ...
        if(emulateEvenOdd || gs.getNonstrokeColor().isPattern() || gs.nonstrokeColorSpace.getID()== ColorSpaces.Pattern) {  //complex stuff
    ...
    protected void drawPatternedShape(Shape currentShape, GraphicsState gs) {
        ...
        if (emulateEvenOdd) {
            ...
            if (clipShape != null)
                g2.clip(clipShape);
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); // Putting above the clip may help. chinese/lect...b22
        }
        ...
        if (cropBoxScaled.intersects(shading)) {
            ...
            if(embedImageAsBase64){
                ...
                // Store image.
                currentPatternedShapeName = customIO.writeImage(rootDir,imageDir+shadeId,img);
            }
    ...
    private boolean isObjectVisible(Rectangle bounds, Area clip) {
        ...
        if(!emulateEvenOdd && dx==0 && dy==0 && dr==0){
...
//            	bounds.translate(-dx, -dy);
	        ...
	        if (cropBox != null) {
	        	double cropStartX = cropBox.getBounds2D().getX();
	            double cropStartY = cropBox.getBounds2D().getY();
	            double cropEndX = cropBox.getBounds2D().getWidth() + cropStartX;
	            double cropEndY = cropBox.getBounds2D().getHeight() + cropStartY;
...
//			lastB=-1;
    ...
    public String getPageAsHTMLRef(int pageNumber) {
        ...
        if(firstPageName !=null && pageNumber==1){
            pageAsString= firstPageName;
        }else{
            //add required zeros
            String maxNumberOfPages = String.valueOf(endPage);
