...
/**/
	...
	/**
	 * method recursively look until which finds the pageArea element
	 * and store the page number and rootNode into pageArea HashMap
	 * @param Node node
			...
			if (nodeType==PAGESET) {
				...
				for(int z=0;z<pageSetChildList.getLength();z++){
					if(getNodeType(pageSetChildList.item(z).getNodeName())==PAGEAREA){
						pageAreaList.add(pageSetChildList.item(z));
					}
				}
				...
				if (pageAreaList.size()==1  && pageSetSiblings.size()==1) {    				
					XFAPageContent pCont = new XFAPageContent();
					pCont.setPageAreaNode(pageAreaList.get(0));
					pCont.getNodeList().add(pageSetSiblings.get(0));
					pageMap.put(1, pCont);    				    				
				}    					
				else {    										
					int pageCount = 1;
					Node currentPageArea = pageAreaList.get(0);
							...
							for(int q=0;q<sibChildList.getLength();q++){
								...
								if(type==BREAK){									
									...
									if(startNewAttr != null){
										if(pageAreaList.size()>1 && pageCount < pageAreaList.size()){
											pageCount++;
										} 
										if (beforeTargetAttr!=null){
											...
											currentPageArea = pageAreaList.get(pageCount-1);
										}   									
									}
									else{
									...
									if(formChild.getAttributes().getNamedItem("startNew")!=null){    	
										if(pageMap.get(pageCount)==null){
											...
											pageMap.put(pageCount,pc);
										}
										else{
											pageMap.get(pageCount).getNodeList().add(sibNode);        							
										}
										pageCount++;
									}
									continue bigLoop;
